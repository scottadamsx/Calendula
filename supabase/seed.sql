...
begin
  ...
  insert into calendula_scheduling_profile (user_id) values (seed_user_id)
  on conflict (user_id) do nothing;

  insert into calendula_attention_profile (user_id) values (seed_user_id)
  on conflict (user_id) do nothing;

  -- Categories.
  insert into calendula_categories (user_id, name) values
  ...
  select id into cat_coursework from calendula_categories where user_id = seed_user_id and name = 'coursework';
  select id into cat_txtsquad   from calendula_categories where user_id = seed_user_id and name = 'txtsquad';
  select id into cat_sjhc       from calendula_categories where user_id = seed_user_id and name = 'sjhc';
  select id into cat_build      from calendula_categories where user_id = seed_user_id and name = 'build';
  select id into cat_admin      from calendula_categories where user_id = seed_user_id and name = 'admin';

  -- Energy windows — apply every day (day_of_week null).
  insert into calendula_energy_windows (user_id, day_of_week, start_time, end_time, quality, label) values
  ...
  insert into calendula_fixed_blocks (user_id, title, starts_at, ends_at, rrule, source) values
  ...
  insert into calendula_fixed_blocks (user_id, title, starts_at, ends_at, high_exertion, rrule, source)
  values (
  ...
  -- admin 60m due +2d.
  insert into calendula_tasks (user_id, category_id, title, estimated_minutes, remaining_minutes, deadline) values
  ...
  insert into calendula_habits (user_id, title, duration_minutes, target_sessions_per_week, preferred_labels) values
    (seed_user_id, 'Gym', 60, 3, array['physical']::calendula_energy_label[]),
    (seed_user_id, 'Violin', 30, 4, array['creative']::calendula_energy_label[]);

  -- People: Maria cadence 1 · Nick cadence 21 last seen 24d · Erin cadence 21 last seen 22d.
  insert into calendula_people (user_id, name, desired_cadence_days, last_interaction_at) values
  ...
  insert into calendula_activity_types (
  ...
  insert into calendula_activity_holds (user_id, activity_type_id, starts_at, ends_at, status)
  values (
  ...
  insert into calendula_placements (user_id, source_type, source_id, title, starts_at, ends_at, hardness)
  values (
  ...
  -- Reminders.
  insert into calendula_reminders (user_id, title, kind, due_at, cost, recurrence) values
    (seed_user_id, 'Subscription renewal', 'moment', date_trunc('month', now()) + interval '6 day' + interval '9 hour', 'ambient', 'monthly');

  insert into calendula_reminders (user_id, title, kind, window_start, window_end) values
    (seed_user_id, 'Call about the gear', 'window', now(), now() + interval '7 day');

  insert into calendula_reminders (
  ...
  select
    ...
    (select id from calendula_people where user_id = seed_user_id and name = 'Maria')
  from calendula_placements p
  where p.source_id = hike_block_id and p.source_type = 'fixed'
  limit 1;

  -- The hike fixed_block above has no matching calendula_placements row yet in Phase 0
  -- (calendula_placements are solver output, and no solver has run) — fall back to a
  -- context reminder with no trigger_placement_id rather than a dangling FK.
  if not exists (
    select 1 from calendula_reminders where user_id = seed_user_id and title = 'Message Dee'
  ) then
    insert into calendula_reminders (user_id, title, kind, trigger, lead_minutes, importance, person_id)
    values (
  ...
  insert into calendula_reminders (user_id, title, kind) values
  ...
  insert into calendula_reminders (
    ...
    'derived', 'calendula_activity_holds', camping_hold_id
  );
end $$;
